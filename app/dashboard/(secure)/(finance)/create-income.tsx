...
export default function IncomeForm() {
    ...
    const validate = () => {
        ...
        if (requireFamilyCount) {
            if (isNaN(familyCount) || familyCount < 0) newErrors.familyCount = "Jumlah KK tidak valid.";
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            await addIncome({
                ...
                note: formData.note || FINANCE_INCOME_SOURCES.find(s => s.id === formData.source)?.note,
            });

            showSuccessToast(
                "Berhasil",
                "Data Penerimaan kas telah disimpan"
            )
            setFormData(INITIAL_FORM)
            router.replace("/dashboard/finance")
        } catch (e: any) {
            console.error("Income submission failed")
            showErrorToast("Gagal Menyimpan", e.message || "Data Penerimaan Kas Gagal Disimpan")
        } finally {
            setSaving(false)
        }
    }
