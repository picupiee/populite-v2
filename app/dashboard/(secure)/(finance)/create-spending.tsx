...
export default function SpendingForm() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof SpendingFormData, string>>
  >({});
  ...
  const validate = () => {
    ...
    if (isNaN(quantity) || quantity < 0)
      newErrors.quantity = "Jumlah Satuan Tidak Valid.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addSpending({
        ...
        note:
          formData.note ||
          FINANCE_SPENDING_TYPES.find((s) => s.id === formData.type)?.note,
      });

      await logActivity(
        "CREATE",
        "FINANCE",
        `Created spending: ${formData.amount} for ${selectedTypeLabel}`,
        undefined,
        {
          type: "SPENDING",
          amount: parseFloat(formData.amount),
          spendingType: formData.type,
          note: formData.note,
        }
      );

      showSuccessToast("Berhasil", "Data Pengeluaran kas telah disimpan");
      ...
      showErrorToast(
        "Gagal Menyimpan",
        e.message || "Data Pengeluaran Kas Gagal Disimpan"
      );
    } finally {
      setSaving(false);
    }
  ...
  const selectedTypeLabel =
    FINANCE_SPENDING_TYPES.find((s) => s.id === formData.type)?.label ||
    "Tipe Pengeluaran";
      ...
      >
        ...
        <SelectGroup
          ...
          onValueChange={(label) => {
            ...
            const id =
              FINANCE_SPENDING_TYPES.find((s) => s.label === label)?.id ||
              label;
